...
begin
  ...
  begin
    ...
    if AForm.MinHeight > 0 then
    begin
    if AForm.MinHeight > readInteger(LINISection, 'Height', LHeight)
    then
    AForm.Height := AForm.MinHeight else
    AForm.Height := readInteger(LINISection, 'Height', LHeight);
    end;

    if AForm.MinWidth > 0 then
    begin
    if AForm.MinWidth > readInteger(LINISection, 'Width', LWidth) then
    AForm.Width := AForm.MinWidth else
    AForm.Width := readInteger(LINISection, 'Width', LWidth);
    end;

    if AForm.MaxHeight > 0 then
    begin
    if AForm.MaxHeight < readInteger(LINISection, 'Height', LHeight) then
    AForm.Height := AForm.MaxHeight else
    AForm.Height := readInteger(LINISection, 'Height', LHeight);
    end;

    if AForm.MaxWidth > 0 then
    begin
    if AForm.MaxWidth > readInteger(LINISection, 'Width', LWidth) then
    AForm.Width := AForm.MaxWidth else
    AForm.Width := readInteger(LINISection, 'Width', LWidth);
    end;
  end;
