{
 ...
 // Modified to work for fpUI-develop branch
 // FredvS
...
begin
{
  ...
  hintlabel.height := Height-(FBorder*2)-6; 
  hintlabel.width := Width-(FBorder*2)-1;
  hintlabel.TextColor := FTextColor;
  hintlabel.text := text;   
end;
...
begin
  ...
  hintlabel := TfpgLabel.Create(self);
  hintlabel.alignment := tacenter;
  hintlabel.ShowHint := false;
 
  hintlabel.top := 7;
  hintlabel.left := 2;  
  hintlabel.fontdesc := fontdesc;  
