{
    ...
    Description:
      ...
      Added support for dynamic loading of dependencies.
}
...
begin
  ...
  xia_Atom_Pair := XInternAtom(xapplication.Display, 'ATOM_PAIR', ord(False));

  // find out how much data there is
  r := XGetWindowProperty(xapplication.Display, e.requestor, e._property, 0,  0, ord(False), AnyPropertyType,
  ...
  begin
    // read the data
    r := XGetWindowProperty(xapplication.Display, e.requestor, e._property, 0, SizeOf(AtomPair), ord(False), AnyPropertyType,
      @Atom, @Format, @Length, @BytesLeft, @Data);
...
begin
  ...
  else if Atom = XInternAtom(xapplication.Display, 'MULTIPLE', ord(False)) then
  begin
    // multiple targets
    HandleMultiple(e);
  end
...
begin
  ...
  XSendEvent(xapplication.Display, e.requestor, ord(False), 0, @e );
end;
...
begin
  ...
  //  WriteLn('KeySym: $', IntToHex(KeySym, 4));
  //  WriteLn('KeySym: ', KeySym);
...
begin
  XdndAware         := XInternAtom(FDisplay, 'XdndAware',         ord(False));
  XdndTypeList      := XInternAtom(FDisplay, 'XdndTypeList',      ord(False));
  XdndSelection     := XInternAtom(FDisplay, 'XdndSelection',     ord(False));

  // client messages
  XdndEnter         := XInternAtom(FDisplay, 'XdndEnter',         ord(False));
  XdndPosition      := XInternAtom(FDisplay, 'XdndPosition',      ord(False));
  XdndStatus        := XInternAtom(FDisplay, 'XdndStatus',        ord(False));
  XdndLeave         := XInternAtom(FDisplay, 'XdndLeave',         ord(False));
  XdndDrop          := XInternAtom(FDisplay, 'XdndDrop',          ord(False));
  XdndFinished      := XInternAtom(FDisplay, 'XdndFinished',      ord(False));

  // actions
  XdndActionCopy    := XInternAtom(FDisplay, 'XdndActionCopy',    ord(False));
  XdndActionMove    := XInternAtom(FDisplay, 'XdndActionMove',    ord(False));
  XdndActionLink    := XInternAtom(FDisplay, 'XdndActionLink',    ord(False));
  XdndActionAsk     := XInternAtom(FDisplay, 'XdndActionAsk',     ord(False));
  XdndActionPrivate := XInternAtom(FDisplay, 'XdndActionPrivate', ord(False));
end;
...
begin
  ...
  XSendEvent(FDisplay, FSrcWinHandle, ord(False), NoEventMask, @Msg);
end;
...
begin
  ...
  ClipboardManager:= XInternAtom(xapplication.Display, 'CLIPBOARD_MANAGER', ord(False));
  ...
  XSync(xapplication.Display, ord(False));
...
begin
  ...
  XSendEvent(xapplication.Display, ATarget, ord(False), NoEventMask, @xev);
end;
...
begin
  ...
  begin
    ...
    XSendEvent(xapplication.Display, FLastTarget, ord(False), NoEventMask, @xev);
...
begin
  ...
  XSendEvent(xapplication.Display, e.requestor, ord(False), NoEventMask, @e );
end;
...
begin
  ...
  selection_atom := XInternAtom(xapplication.Display, buf, ord(False));
...
begin
  ...
  ev.xclient.message_type := XInternAtom(xapplication.Display, '_NET_SYSTEM_TRAY_OPCODE', ord(False) );
  ...
  Result := XSendEvent(xapplication.Display, TrayIconParent, ord(False), NoEventMask, @ev) <> 0;
  XSync(xapplication.Display, ord(False));
end;
