...
begin
  for ANetAtom := Low(TNetAtomEnum) to High(TNetAtomEnum) do begin
    FNetAtoms[ANetAtom] := XInternAtom(FDisplay, PChar(NetAtomStr[ANetAtom]), ord(False))
  end;
  UTF8_STRING := XInternAtom(FDisplay, 'UTF8_STRING', ord(False));
end;
...
begin
  ...
  Result := XSendEvent(FDisplay, AWindow, ord(APropagate), AMask, AMessage);
end;
