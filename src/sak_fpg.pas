...
begin
  ...
  if assigned(InitSpeech) then
   ...
   {$ifdef windows}
    ...
    if fileexists(ordir + 'libportaudio.so') then
    begin
      Result := 0;
      initspeech.PA_FileName := ordir + 'libportaudio.so';
    end
    else
    if fileexists(ordir + '/sakit/libfreebsd32/libportaudio.so') then
    begin
      initspeech.PA_FileName := ordir + '/sakit/libfreebsd32/libportaudio.so';
      Result := 0;
    end;
...
begin
  ...
  begin
    ...
    with (fpgapplication.Forms[f]) as TfpgForm do
      ...
      begin
        if (Components[i] is TfpgButton) or (Components[i] is TfpgMemo) or (Components[i] is TfpgEdit) or
