{ This is a patched version of main.pp for Lazarus version 1.2.6
It allows to use fpGUI Designer inside Lazarus IDE.
Fred van Stappen
fiens@hotmail.com
}
...
type
  ...
  TMainIDE = class(TMainIDEBase)
    ...
    procedure OnDesigner_extTimer(Sender: TObject);
...
begin
  ...
  {$IFDEF IDE_MEM_CHECK}CheckHeapWrtMemCnt('TMainIDE.Create INHERITED');{$ENDIF}
   ...
   //// Fred was here
  FDEFileName := '' ;
  FDesigner_extTimer:=TTimer.Create(self);
  FDesigner_extTimer.Interval:=600;
  FDesigner_extTimer.OnTimer:=@OnDesigner_extTimer;
  FDesigner_extTimer.Enabled:=false;
   ////
...
//// fred was here for fpgui
 procedure TMainIDE.LoadfpgDesigner(const AfpgFilename: string; param : string);
 var
   f : textfile ;
   dataf : string ;
    x : integer ;
 ...
 begin
     ...
     dataf := copy(GetAppConfigDir(false),1, pos(applicationname, GetAppConfigDir(false))-1) +
         'designer_ext' + directoryseparator + 'designer_ext.ini'  ;
   ...
   if fileexists(pchar(dataf)) then
      ...
      while (not eof(f)) and (x = 0) do
        begin
          Readln(F, dataf);
          if   Pos('Application=',dataf) > 0 then x := 1;
          end;
         CloseFile(f);
   if dataf <> '' then dataf := copy(dataf, Pos('Application=',dataf) +12 , length(dataf) - Pos('Application=',dataf) +12) ;

    if fileexists(pchar(dataf)) then
      begin
        AProcess := TProcess.Create(nil);
       {$WARN SYMBOL_DEPRECATED OFF}
       AProcess.CommandLine := dataf + ' ' + AfpgFilename +  param;
...
begin
 ...
 ///// fred was here for fpgui
 FDesigner_extTimer.Enabled:=false;
   FDEFileName :=  AFilename ;
   FDesigner_extTimer.Enabled:=true;
 //////
end;
...
begin
  ...
  DoSaveEditorFile(SrcEdit, [sfCheckAmbiguousFiles]);
   ///// fred was here for fpgui
 FDesigner_extTimer.Enabled:=false;
if (FDEFileName = SrcEdit.FileName) then
begin
   FDEFileName :=  SrcEdit.FileName ;
   FDesigner_extTimer.Enabled:=true;
end;
 //////
 end;
...
//// Fred was here
procedure TMainIDE.OnDesigner_extTimer(Sender: TObject);
begin
 FDesigner_extTimer.Enabled:=false;
  LoadfpgDesigner(FDEFileName, '');
end;
///
